...
UNIVERSAL_NEW_LINE_PATTERN = re.compile(r'''((?:\r
)|(?:\r)|(?:
))''')
QUOTE_PATTERN = \
    re.compile('([rRuU]{,2})((?:"{3})|(?:\'{3})|(?:")|(?:\'))')  # 2007 May 01
...
def force_quote(encoded, double=True, quoted=True):  # 2007 May 01
    """Change the type of quotation marks (or not) on an already quoted string.
    ...
    for (ndx, substring) in \
        enumerate(double_backslash_delimited_substrings):
    ...
    def __init__(self):
        ...
        for (token_type, token_string, start, end, line) in lines:
            ...
            if token_type in [tokenize.COMMENT, tokenize.NL]:
                ...
                try:
                    original = \
                        token_string.strip().decode(INPUT.coding,
                            'backslashreplace')
                    ...
                    if encoded == original:
                        ...
                        original_values = \
                            self.literal_pool.setdefault(decoded, [])  # 2007 May 01
    ...
    def merge(self, lineno=None, fin=False):
        ...
        if text and WRAP_DOC_STRINGS:  # 2007 May 25
            ...
            if text:
                ...
                line_length = COL_LIMIT - (save_col
                         + len(COMMENT_PREFIX))
    ...
    def rept_collision(self, key):
        ...
        if len(self) == 1:
            ...
            sys.stderr.write("Error:  %s ambiguously replaced by '%s' at line %i.\n"
                              % (str(self), self.new, OUTPUT.lineno
                              + 1))
...
def transform(indent, lineno, node):
    ...
    *compiler* module to objects with *put* methods.
    ...
    if isinstance_(node, 'Add'):
        ...
        result = NodeListCompFor(indent, lineno, node.assign,
                                 node.list, node.ifs)
        ...
        result = NodeTryExcept(indent, lineno, node.body,
                               node.handlers, node.else_)
        ...
        result = NodeWith(indent, lineno, node.expr, node.vars,
                          node.body)
...
class Node(object):
    ...
    def put(self, can_split=False):
        ...
        self.line_more(' /* %s at line %i */ ' % (self.tag,
                       self.get_lineno()))
        return self
...
class NodeOpr(Node):
    ...
    def put_expr(self, node, can_split=False):
        if type(node) in OPERATOR_TRUMPS[type(self)] or type(node)\
             in OPERATOR_LEVEL[type(self)] and not self.is_commutative:
...
class NodeStr(Node):
    ...
    def put_doc(self, need_blank_line=ZERO):
        ...
        if LEFTJUST_DOC_STRINGS:
            ...
            margin = '%s%s' % (OUTPUT.newline, INDENTATION
                                * self.indent)  # 2006 Dec 05
        ...
        if WRAP_DOC_STRINGS:  # 2007 May 25
            ...
            line_length = max(line_length, 20)
            lines = wrap_lines(doc.strip().splitlines(),
                               width=line_length)
            lines.extend([NULL, NULL])
            doc = margin.join(lines)
...
class NodeAsgAttr(NodeOpr):
    ...
    def put(self, can_split=False):
        ...
        self.line_more('.')
        self.line_more(NAME_SPACE.make_attr_name(self.expr,
                       self.attrname))
        if DEBUG:
...
class NodeDict(Node):
    ...
    def __init__(
        ...
        Node.__init__(self, indent, lineno)
        self.items = [(transform(indent, lineno, key),
                      transform(indent, lineno, value)) for (key,
                      value) in items]
        return
...
class NodeFunction(Node):
    ...
    def __init__(
        ...
        self.defaults = [transform(indent, lineno, default)
...
class NodeTryExcept(Node):
    ...
    def __init__(
        ...
        Node.__init__(self, indent, lineno)
        self.body = transform(indent + 1, lineno, body)
        self.handlers = [(transform(indent, lineno, expr),
